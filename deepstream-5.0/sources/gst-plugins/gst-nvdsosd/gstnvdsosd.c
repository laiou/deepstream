...
static gboolean gst_nvds_osd_parse_hw_blend_color_attrs (GstNvDsOsd * nvdsosd,
    const gchar * arr);
static gboolean gst_nvds_osd_get_hw_blend_color_attrs (GValue * value,
    GstNvDsOsd * nvdsosd);
...
gst_nvds_osd_start (GstBaseTransform * btrans)
{
  ...
  nvll_osd_init_colors_for_hw_blend (nvdsosd->nvdsosd_context,
      nvdsosd->color_info, nvdsosd->num_class_entries);
    ...
    const GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_HW_BLEND_COLOR_ATTRS:
      ...
      gst_nvds_osd_parse_hw_blend_color_attrs (nvdsosd,
...
static gboolean
gst_nvds_osd_parse_hw_blend_color_attrs (GstNvDsOsd * nvdsosd,
    const gchar * arr)
{
  gchar *str = (gchar *) arr;
  int idx = 0;
...
static gboolean
gst_nvds_osd_get_hw_blend_color_attrs (GValue * value, GstNvDsOsd * nvdsosd)
{
  int idx = 0;
  gchar arr[100];
  ...
  while (idx < (nvdsosd->num_class_entries - 1)) {
    sprintf (arr, "%d,%f,%f,%f,%f:",
        nvdsosd->color_info[idx].id, nvdsosd->color_info[idx].color.red,
        nvdsosd->color_info[idx].color.green,
        nvdsosd->color_info[idx].color.blue,
        nvdsosd->color_info[idx].color.alpha);
    idx++;
...
GST_PLUGIN_DEFINE (GST_VERSION_MAJOR,
    ...
    nvdsgst_osd,
    PACKAGE_DESCRIPTION,
    nvdsosd_init, "5.0", PACKAGE_LICENSE, PACKAGE_NAME, PACKAGE_URL)
